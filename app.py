...
if archivo:
    ...
    iconos_por_tecnico = {
        ...
        "SIN": "https://drive.google.com/uc?export=view&id=1R3XU3e_default_icon_id"  # ícono por defecto
    }
    ...
    for tramo in df['Tramo'].unique():
        ...
        grupo = FeatureGroup(name=f"{emoji} {tramo}")
        grupos_tramos[tramo] = grupo
        mapa.add_child(grupo)

    # Aseguramos grupo "SIN TRAMO"
    if "SIN TRAMO" not in grupos_tramos:
        grupo_default = FeatureGroup(name="❓ SIN TRAMO")
        grupos_tramos["SIN TRAMO"] = grupo_default
        mapa.add_child(grupo_default)

    # Grupos por técnico
    grupos_tecnicos = {}
    for codigo in df['CodigoTecnico'].unique():
        grupo = FeatureGroup(name=f"🛠️ Técnico {codigo}", show=False)
        grupos_tecnicos[codigo] = grupo
        mapa.add_child(grupo)

    if "SIN" not in grupos_tecnicos:
        grupo_default = FeatureGroup(name="🛠️ Técnico SIN", show=False)
        grupos_tecnicos["SIN"] = grupo_default
        mapa.add_child(grupo_default)
    ...
    for _, row in df.iterrows():
        ...
        grupo_tramo = grupos_tramos.get(row['Tramo'], grupos_tramos["SIN TRAMO"])
        grupo_tramo.add_child(marker)

        grupo_tecnico = grupos_tecnicos.get(row['CodigoTecnico'], grupos_tecnicos["SIN"])
        grupo_tecnico.add_child(marker)
